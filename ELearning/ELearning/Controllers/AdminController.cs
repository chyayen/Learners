    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult Login(LoginViewModel model)
        {
            ...
            if (userdb != null && !string.IsNullOrEmpty(userdb.UserName))
            {
                ...
                {
                    ViewData["AlertMessage"] = $"<p class='alert alert-danger'>Account locked. Try again after {lockOutEndTime.ToString("g")}</p>";
                    return View(model);
                }
                else
                {
        ...
        private AdminViewModel GetUserDataForDashboard(int userid, string usertype)
        {
            ...
            if (usertype == "teacher")
            {
                cmd.CommandText = @"select *, (select COUNT(*) from studentclasses sc inner join sections s on sc.sectionid = s.id and s.teacherid = u.id) as TotalStudents 
            ...
            {
                cmd.CommandText = @"select *, (select COUNT(*) from studentclasses sc inner join sections s on sc.sectionid = s.id) as TotalStudents 
	                                , (select COUNT(*) from stories s 
                                        left join classes c on s.classid = c.id 
                                        left join sections sec on sec.classid = c.id
        ...
        private int UserInsertDB(RegisterViewModel model)
        {
            ...
            { 
                ...
                {
                    ViewData["AlertMessage"] = "<p class='alert alert-danger'>Username already exist.</p>";
                }
                else
                {
                    ViewData["AlertMessage"] = "<p class='alert alert-danger'>" + ex.Message + "</p>";
        ...
        private ResultModel SaveStoryDB(StoryModel model, string action)
        {
            ...
            {
                cmd.CommandText = "INSERT INTO `stories`(`title`, `content`, `classid`, `addedby`, `dateadded`, `updatedby`, `dateupdated`, `isdeleted`) VALUES (@title,@content,@classid,@addedby,@dateadded,@updatedby,@dateupdated,0);SELECT LAST_INSERT_ID();";
                cmd.Parameters.AddWithValue("@title", model.Title);
                cmd.Parameters.AddWithValue("@content", model.Content);
                cmd.Parameters.AddWithValue("@classid", model.ClassID);
        ...
        private List<LatestStudentSubmissionQuestionScoreModel> GetLatestStudentSubmissionDB(int storyid, int studentid, int attempt)
        {
            ...
            MySqlConnection con = new MySqlConnection(defaultConnection);
            MySqlCommand cmd = con.CreateCommand();
            cmd.CommandText = @"select sa.id as answerid, sa.questionid, sq.question, sa.answer, sa.attempt, sa.dateanswered, sa.studentid
        ...
        private int InsertStudentGrade(int studentid, int storyid, int attempt, int totalquestions, decimal grade, string remarks, int checkby, DateTime datechecked)
        {
            ...
            cmd.CommandText = @"INSERT INTO `studentgrades`(`studentid`, `storyid`, `attempt`, `totalquestions`, `grade`, `remarks`, `checkedby`, `datechecked`) 
        ...
        private List<DetailedQuizGradeByStudentModel> GetDetailedQuizResultByStoryDB(int studentid, int storyid, int attempt)
        {
            ...
            cmd.CommandText = @"SELECT sa.questionid, sq.question, sa.answer, sa.dateanswered, sa.score, sg.storyid, sg.studentid, sa.attempt, sg.totalquestions, sg.grade, sg.remarks, sg.checkedbyname, sg.datechecked
                                ...
                                where sa.attempt = @attempt and sa.studentid = @studentid
